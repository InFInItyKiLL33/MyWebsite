...
function App() {
  ...
  const [allowedContentTypes, setAllowedContentTypes] = useState([
    -2, -2, -2, -2,
  ]); // -3 backend error, -2 for loading, -1 for access all, 0 for no access (all 0 for invalid uuid), 1 and above for certain access
        ...
        if (response.status === 200 && response.data["result"] === "ok") {
          ...
          for (let i = 0; i < Object.keys(TYPES).length; i++) {
            setRetrievedContent((prevRetrievedContent: any): any => {
              return [
                ...prevRetrievedContent,
                response.data["content" + String(i + 1)],
              ];
            });
          }
        } else if (
          response.data["result"] === "error" ||
          response.data["result"] === "excessive"
        ) {
          setAllowedContentTypes([-3, -3, -3, -3]);
  ...
  return (
    <>
      <Routes>
        <Route
          ...
          element={
            <Home
              ...
              setAllowedContentTypes={setAllowedContentTypes}
              types={TYPES}
              getCookie={getCookie}
            />
          }
        ...
        <Route path="/linkedin" element={<LinkedIn page="5" />} />
      </Routes>
    </>
  );
}
